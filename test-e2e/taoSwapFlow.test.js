...
if [ -n "\${PHASE8_LN_LOG:-}" ]; then
  printf '{"cmd":"%s","ts":%s}\\n' "$cmd" "$(date +%s)" >> "$PHASE8_LN_LOG"
fi

peer="\${PHASE8_PEER:-}"
preimage="\${PHASE8_PREIMAGE:-}"
...
test('e2e: deterministic TAO settlement swap flow through prompt executor', async (t) => {
  const signer = await createSigner();
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'intercomswap-phase8-tao-'));
  ...
  const prevLnLog = process.env.PHASE8_LN_LOG;
  const prevPreimage = process.env.PHASE8_PREIMAGE;
  const prevPeer = process.env.PHASE8_PEER;
  ...
  process.env.PHASE8_LN_LOG = lnLogPath;
  process.env.PHASE8_PREIMAGE = preimageHex;
  process.env.PHASE8_PEER = signer.pubHex;

  t.after(() => {
    if (prevLnLog === undefined) delete process.env.PHASE8_LN_LOG;
    else process.env.PHASE8_LN_LOG = prevLnLog;
    if (prevPreimage === undefined) delete process.env.PHASE8_PREIMAGE;
    else process.env.PHASE8_PREIMAGE = prevPreimage;
    if (prevPeer === undefined) delete process.env.PHASE8_PEER;
    else process.env.PHASE8_PEER = prevPeer;
    try {
  ...
  const mockSettlementId = `0x${'ab'.repeat(32)}`;
  const mockLockTxId = `0x${'cd'.repeat(32)}`;
  const mockClaimTxId = `0x${'ef'.repeat(32)}`;
  ...
  const mockSettlementProvider = {
    ...
    async verifySwapPrePayOnchain(input) {
      ...
      if (!input?.escrowBody?.settlement_id) return { ok: false, error: 'missing settlement_id' };
      return {
        ok: true,
        error: null,
        onchain: {
          state: {
            settlementId: input.escrowBody.settlement_id,
          },
        },
      };
    },
  ...
  assert.equal(quoteAcceptRes.quote_id, quoteId);
  assert.equal(quoteAcceptRes.envelope.kind, KIND.QUOTE_ACCEPT);
  ...
  assert.equal(escrowRes.envelope.body.settlement_id, mockSettlementId);
  assert.equal(escrowRes.envelope.body.tx_id, mockLockTxId);
  ...
  const verifyCall = settlementCalls.find((row) => row.op === 'verify');
  assert.ok(verifyCall, 'verifySwapPrePayOnchain was not called');
  ...
  assert.equal(lnPayRes.type, 'ln_paid_posted');
  assert.equal(lnPayRes.preimage_hex, preimageHex);
  assert.equal(countLnPayCalls(lnLogPath), 1);

  const claimRes = await executor.execute(
  ...
  assert.equal(claimRes.type, 'tao_claimed_posted');
  assert.equal(claimRes.envelope.kind, KIND.TAO_CLAIMED);
  assert.equal(claimRes.envelope.body.settlement_id, mockSettlementId);
  assert.equal(claimRes.envelope.body.tx_id, mockClaimTxId);

  const callOps = settlementCalls.map((row) => row.op);
  assert.ok(callOps.indexOf('verify') !== -1, 'verify call missing');
  assert.ok(callOps.indexOf('claim') !== -1, 'claim call missing');
  assert.ok(callOps.indexOf('verify') < callOps.indexOf('claim'), 'claim happened before verify');

  let trade = createInitialTrade(tradeId);
  for (const envelope of [termsEnvelope, acceptEnvelope, invoiceEnvelope, escrowRes.envelope, lnPayRes.envelope, claimRes.envelope]) {
    const step = applySwapEnvelope(trade, envelope);
    assert.equal(step.ok, true, step.error || 'state machine failed');
    trade = step.trade;
  }
  assert.equal(trade.state, STATE.CLAIMED);
  ...
  try {
    ...
    assert.equal(receipt.tao_claim_tx_id, mockClaimTxId);
  } finally {
    store.close();
  }
});
