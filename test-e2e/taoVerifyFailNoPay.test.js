...
if [ -n "\${PHASE9_VERIFY_FAIL_LN_LOG:-}" ]; then
  printf '{"cmd":"%s","ts":%s}\\n' "$cmd" "$(date +%s)" >> "$PHASE9_VERIFY_FAIL_LN_LOG"
fi
...
test('e2e: TAO verify failure aborts before LN pay and no claim is emitted', async (t) => {
  const signer = await createSigner();
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'intercomswap-phase9-tao-verify-fail-'));
  ...
  const prevLnLog = process.env.PHASE9_VERIFY_FAIL_LN_LOG;
  process.env.PHASE9_VERIFY_FAIL_LN_LOG = lnLogPath;

  t.after(() => {
    if (prevLnLog === undefined) delete process.env.PHASE9_VERIFY_FAIL_LN_LOG;
    else process.env.PHASE9_VERIFY_FAIL_LN_LOG = prevLnLog;
    try {
  ...
  const tradeId = 'phase9-tao-verify-fail-trade';
  const channel = 'swap:phase9-tao-verify-fail-trade';
  ...
  executor._getSettlementProvider = function getSettlementProviderMock() {
    return {
      async verifySwapPrePayOnchain() {
        return { ok: false, error: 'forced verify failure for test' };
      },
      async claim() {
        claimCalls += 1;
  ...
  const invoiceEnvelope = signEnvelope(
    createUnsignedEnvelope({
      ...
      tradeId,
      body: {
        bolt11: 'lnbcrt1phase9verifyfailinvoice0000000000001',
  ...
  const escrowEnvelope = signEnvelope(
    createUnsignedEnvelope({
      ...
      body: {
        ...
        refund_after_unix: nowUnix + 7200,
  ...
  assert.ok(err, 'expected verify failure');
